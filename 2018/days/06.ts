...
export default (): void => {
  ...
  coordsBoundary.forEach((edge) => {
    ...
    coordinates
      .filter((c) => c.x === minDistance.x && c.y === minDistance.y)
      .forEach((o) => (o.infinite = true));
  });

  const gridArea: IGridArea = {};
  let regionArea = 0;
  ...
  for (let y = minPoint.y; y <= maxPoint.y; y++) {
    for (let x = minPoint.x; x < maxPoint.x; x++) {
      const point = { x, y };
      const minDistances = coordinates
        .map((o) => {
      ...
      const sumDistance = minDistances.reduce(
      ...
      if (minDistances[0].area !== minDistances[1].area) {
        // means we have a single coordinate with the shortest manhattan distance
  ...
  const areas = Object.values(gridArea)
    .filter((o) => !o.infinite)
    .map((o) => o.area);
...
const getBoundaryCoordinates = (
  minCoord: ICoordinate,
  maxCoord: ICoordinate
): ICoordinate[] => {
  const coordinates = [];
...
const calculateManhattanDistance = (
  ...
  Math.abs(coordinate.y - point.y) + Math.abs(coordinate.x - point.x);
