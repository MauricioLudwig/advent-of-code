...
export default (): void => {
  const input = getAsArray('02.txt').map((o): number[] =>
    o.split('\t').map(Number)
  );

  const checkSum1 = input.reduce((acc, curr): number => {
  ...
  const checkSum2 = input.reduce((acc, curr): number => {
    let num = 0;
    let abort = false;
    const numbers = curr.sort((a, b): number => b - a);
